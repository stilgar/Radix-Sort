...
void radixsort_parallel(uint32_t *input_array, uint32_t input_array_size,
				uint32_t max_value, int debug, int chunksize)
{
	uint32_t digit, tmp_value;
	int f, i, j;
	int max_lenght=0;
	double start_time;
	...
	if (!tmp) {
        ...
        /* 1) it calculates max number of digit in max_value [K] */
	if (debug > 0) {
        ...
        max_lenght++;
	if (debug > 0) {
		printf("  1) CALCULATION OF K\t\t\t%lf\n",
						    omp_get_wtime()-start_time);
	}

	/* Shared buckets definition */
	if (debug > 0)
		start_time = omp_get_wtime();
	int shared_buckets[max_lenght][DIGITS_AMOUNT];
...
{
	uint32_t i, *input, num_of_values = 1000, max_value = 0;
	int value_lenght=5;
	double start_time;
	char str[10] = {0};
	int chunksize=1;
	...
	if (argc >= 4) {
		int tmp_value;
		if ((tmp_value=atoi(argv[3]))>0) {
			debug=tmp_value;
		}
	}
	if (argc >= 5) {
		int tmp_value;
		if ((tmp_value=atoi(argv[4]))>0) {
			chunksize=tmp_value;
		}
	}
	...
	start_time = omp_get_wtime();
	radixsort_parallel(input, num_of_values, max_value, debug, chunksize);
	if (debug > 0)
		printf("K\t\tN\t\tTIME\n");
	printf("%i\t%8u\t\t%f\n", value_lenght, num_of_values,
						  omp_get_wtime() - start_time);
